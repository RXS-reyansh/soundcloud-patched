...
var _sc;
var _SoundCloudPlugin = class _SoundCloudPlugin extends import_distube.ExtractorPlugin {
  constructor(options = {}) {
    super();
    __privateAdd(this, _sc);
    ...
    __privateSet(this, _sc, new import_soundcloud.default({
  ...
  async resolve(url, options) {
    ...
    const data = await __privateGet(this, _sc).resolve.getV2(url, true).catch((e) => {
...
_sc = new WeakMap();
__name(_SoundCloudPlugin, "SoundCloudPlugin");
var SoundCloudPlugin = _SoundCloudPlugin;
var _SoundCloudTrack = class _SoundCloudTrack {
  constructor(info) {
...
__name(_SoundCloudTrack, "SoundCloudTrack");
var SoundCloudTrack = _SoundCloudTrack;
var _SoundCloudPlaylist = class _SoundCloudPlaylist {
  constructor(info) {
    ...
    if (!info.tracks?.length) throw new import_distube.DisTubeError("SOUNDCLOUD_PLUGIN_EMPTY_PLAYLIST", "Playlist is empty.");
    this.songs = info.tracks.map((s) => new import_distube.Song(new SoundCloudTrack(s)));
  }
};
__name(_SoundCloudPlaylist, "SoundCloudPlaylist");
var SoundCloudPlaylist = _SoundCloudPlaylist;
var index_default = SoundCloudPlugin;
